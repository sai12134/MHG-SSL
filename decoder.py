...
class Model_decoder(nn.Module):
    ...
    def __init__(self, hidden_size, ablation, device, dropout=0.2):
    ...
    def topo_pred(self, mol_batch, node_rep, super_node_rep):
        ...
        for mol_index, mol in enumerate(mol_batch):
            ...
            if num_bonds < 1:
                ...
                bond_if_input = torch.cat([mol_atom_rep_proj.repeat(1, num_atoms).view(num_atoms * num_atoms, -1),
                                           mol_atom_rep_proj.repeat(num_atoms, 1)], dim=1).view(num_atoms, num_atoms,
                                                                                                -1)
                bond_if_pred = self.bond_if_s(bond_if_input).squeeze(-1)
                a = torch.zeros(num_atoms, num_atoms)
                bond_if_target = a.index_put(indices=[mol.edge_index_nosuper[0, :], mol.edge_index_nosuper[1, :]],
                                             values=torch.tensor(1.)).to(self.device)
                bond_if_loss += self.bond_pred_loss(bond_if_pred, bond_if_target)
                bond_if_auc += roc_auc_score(bond_if_target.flatten().cpu().detach(),
                                             torch.sigmoid(bond_if_pred.flatten().cpu().detach()))
                bond_if_ap += average_precision_score(bond_if_target.cpu().detach(),
                                                      torch.sigmoid(bond_if_pred.cpu().detach()))
        ...
        if self.ablation == "w/o_node":
            bond_if_loss = bond_if_loss * 0
            bond_type_loss = bond_type_loss * 0
            atom_type_loss = atom_type_loss * 0
        ...
        elif self.ablation == "w/o_graph":
            atom_num_loss = atom_num_loss * 0
            bond_num_loss = bond_num_loss * 0
            atom_num_rmse = 0
            bond_num_rmse = 0

        loss_tur = [bond_if_loss / mol_num, bond_type_loss / mol_num, atom_type_loss / mol_num, atom_num_loss,
                    bond_num_loss]
        results = [bond_if_auc / mol_num, bond_if_ap / mol_num, bond_type_acc / mol_num, atom_type_acc / mol_num,
                   atom_num_rmse, bond_num_rmse]
